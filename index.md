...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // All injection attempts will require an "or" statement
    ...
    // checks for threat string within a given command, positive yields an int
    try {
...
bool run_query_injection(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    { // this sql has a where clause
        if (localCopy.back() == ';')
        { // removes semicolon to allow injection to be appended, semicolon is re-appended during attack text
...
// Injection attempts will produce exceptions if handled well, readable DB results if not
...
#####	My second artifact is a SQL injection detector built in C++ that I originally created for CS-405 in early 2021.  While the program itself is mainly concerned with the building of a sample SQLite database, the detection portion shows off a simple text filtration method for testing user input for potential SQL injection attacks.  I began to develop an interest in information security almost from day one in my secure coding class, and really wanted to include an artifact from it in my portfolio to cover another area of interest that I discovered in my time at SNHU.  
...
#####	This artifact demonstrates the following outcomes in particular: “Design and evaluate computing solutions that solve a given problem using algorithmic principles and computer science practices and standards appropriate to its solution, while managing the trade-offs involved in design choices” and “Develop a security mindset that anticipates adversarial exploits in software architecture and designs to expose potential vulnerabilities, mitigate design flaws, and ensure privacy and enhanced security of data and resources”.  I believe the software engineering and design outcome to be represented here as well, although my previous artifact demonstrates it more concisely than this one does.  
...
#####	I found this artifact initially more challenging to enhance not due the complexity of the program, but due to its smaller scope.  At its base, this program is a demo showing how text entry fields can be guarded from a specific type of attack.  I consider the final product to be extremely successful in that regard and am pleased not to have tacked any unnecessary functionality that might muddy its purpose.  Much like my previous artifact did with IoT devices, I've included this program to showcase my affinity for information security, a field that I look forward to learning more about and participating in.
...
#       and allows access the CRUD functions belonging to the class                 ##
######################################################################################
